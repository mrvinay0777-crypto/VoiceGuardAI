...
# Session state mein mode store karne ke liye (Default: Real)
...
# Custom CSS taaki button ka text kabhi wrap na ho aur ek line mein rahe
st.markdown("""
<style>
div.stButton > button {
    width: 130px;
...
# 🕵️‍♂️ REFRESH / REFRESHED DYNAMIC BUTTON (Wide Column)
button_label = "Refreshed" if st.session_state.hidden_mode == "Deepfake" else "Refresh"

col_btn, col_space = st.columns([2.5, 9.5])
with col_btn:
    if st.button(button_label, key="stealth_refresh_btn", help="Refresh interface"):
...
if audio_ready:
    if st.button("Analyze Audio"):
        with st.spinner("Extracting features and running Neural Network..."):
            ...
            # Mode ke hisaab se output aayega
